...
def pad_array(arr, atom_padding):
    return np.pad(
        arr,
...
def parse():
    ...
    for path in paths:
        ...
        for i, (c, a) in enumerate(zip(cartesians_list, atomic_nums_list)):
            ...
            atomic_nums_list[i] = pad_array(a, 21)
        arrays[path.stem] = [np.array(arr) for arr in (cartesians_list, atomic_nums_list)]
    ...
    with h5py.File('./isomerization_dataset.hd5f', 'w') as file:
        for name, (cartesians, atomic_nums) in arrays.items():
            ...
            file.create_dataset(f'{name}/atomic_nums', data=np.nan_to_num(np.squeeze(atomic_nums, axis=-1)))
