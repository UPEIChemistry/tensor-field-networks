...
class ISO17DataLoader(DataLoader):
    ...
    def load_data(self, *args, **kwargs):
        ...
        """
        if self._data is not None and 'dataset_type' not in kwargs and 'test_type' not in kwargs:
            return self._data
        ...
        with h5py.File(self.path, 'r') as file:
            for name, l in data.items():
                positions = self.pad_along_axis(np.array(file['{}/positions'.format(name)]),
                                                self.num_atoms)
                atomic_nums = self.pad_along_axis(np.tile(
                        np.expand_dims(
                            file['{}/atomic_numbers'.format(name)],
                            axis=0
                        ),
                        (len(positions), 1)
                    ), self.num_atoms)
        ...
        # Remapping
        if self.map_atoms:
            [self.remap_atoms(d[1]) for d in data.values()]
