...
class HyperFactory(HyperModel):
    ...
    def _initialize_builder(self, radial_factory: RadialFactory):
        self.builder: Builder = self.builder_cls(
            ...
            num_points=self.kwargs.get("num_points"),
            mu=self.kwargs.get("mu", None),
            sigma=self.kwargs.get("sigma", None),
            trainable_offsets=self.make_hyperparameter(
                "trainable_offsets", fconf["trainable_offsets"]
            ),
            embedding_units=self.make_hyperparameter(
                "embedding_units", fconf["embedding_units"]
            ),
            radial_factory=radial_factory,
            residual=self.make_hyperparameter("residual", fconf["residual"]),
            num_layers=self.make_hyperparameter(
                "model_num_layers", fconf["num_layers"]
            ),
            ...
            sum_points=self.kwargs.get("sum_points", fconf["sum_points"]),
            basis_type=self.make_hyperparameter("basis_type", fconf["basis_type"]),
            basis_config=self.kwargs.get(
                "basis_config",
                {
                    "width": self.hp.Fixed(
                        "basis_width", fconf["basis_config"]["width"]
                    ),
                    "spacing": self.hp.Fixed(
                        "basis_spacing", fconf["basis_config"]["spacing"]
                    ),
                    "min_value": self.hp.Fixed(
                        "basis_min", fconf["basis_config"]["min_value"]
                    ),
                    "max_value": self.hp.Fixed(
                        "basis_max", fconf["basis_config"]["max_value"]
                    ),
                },
            ),
            num_final_si_layers=self.make_hyperparameter(
                "num_final_si_layers", fconf["num_final_si_layers"]
            ),
            final_si_units=self.make_hyperparameter(
                "final_si_units", fconf["final_si_units"]
            ),
        )
        return self.builder
    ...
    def _initialize_radial_factory(self):
        radial_kwargs = dict(
            ...
            kernel_lambda=self.kwargs.get("kernel_lambda", 0.0),
            bias_lambda=self.kwargs.get("bias_lambda", 0.0),
        )
        return get_radial_factory(
            self.kwargs.get("radial_factory", fconf["radial_factory"]), radial_kwargs
        )

    def _compile_model(self, model):
        loss_weights = [
            self.hp.Fixed("{}_weight".format(n), 1) for n in model.output_names
        ] or None
        model.compile(
